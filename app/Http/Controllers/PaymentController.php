...
class PaymentController extends Controller
{
       ...
       public function __construct()
    {
        ...
        $this->middleware('permission:view-payment', ['only' => ['awaitingConfirmation']]);
         
         $this->middleware('permission:approve-payment', ['only' => ['save']]);
    }
  ...
  public function awaitingConfirmation(){
    ...
    $pending->map(function ($transaction) use ($files, $applicant) {
          $value = $files->where('teller_no', $transaction->teller_no)->last();
      ...
      public function decline(Request $request)
  {
    try{
      ...
      $this->name = $input['last'] .' '. $input['first'];
      $this->reason = $input['reason'];
      $this->email = $checkReg->applicant_email;
      RegistrationFile::where('teller_no', $checkReg->teller_no)->update(['validated' => 2]);
      $checkReg->update(['payment_status' => 2]);
      Mail::to($this->email)->send(new DeclinedPayment(
          $this->name,
          $this->reason,
          $this->email
      ));
          return api_request_response(
            "ok",
            "Aplication Declined Successfully!",
            success_status_code(),
            $checkReg
        );
    }
