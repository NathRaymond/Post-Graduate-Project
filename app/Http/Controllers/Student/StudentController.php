...
class StudentController extends Controller
{
    ...
    public function studentregistration(){
        ...
        $data['certificates'] = StudentCertificate::where('applicant_id', Auth::user()->applicant_id)->get();
        $data['olevels'] = StudentOLevel::where('applicant_id', Auth::user()->applicant_id)->select('applicant_id','exam_type','reg_number','year')->distinct()->get();
    ...
    public function create_refreereg(Request $request){
        ...
        $application = Application::where('applicant_id',$input['applicant_id'])->first();
        $user = StudentReferee::create($input);

        $applicant = $request->all();
        $applicant = Applicant::where('id', Auth::user()->applicant_id)->first();
        $applicantdata['is_complete'] = 8;
        $applicant->update($applicantdata);

        //Mail sending

        $data['refname'] = $request->title.' '.$request->surname.' '. $request->firstname;
        $data['ref_email'] = $request->email;
        $data['applicant_name'] =Auth::user()->name;
        $data['application_programme'] =   $application->programmes->description;
        $data['application_course'] =  $application->course->description;
        $data['formlink'] = 'http://127.0.0.1:8000/referee/'.$application->applicationRefNo;
        $data['phone_number'] = $request->phone_number;

        Mail::send('email.referee', $data, function($message)use($data) {
            $message->to($data['ref_email'])
                  ->subject('REFEREE QUESTION');
        });
    ...
    public function create_olevelreg(Request $request){
        ...
        $input['applicant_id'] = Auth::user()->applicant_id;
        $grade = $input['grade'];
        foreach($grade as $key => $score){
            $user = new StudentOLevel;
            $user->grade = $score;
            $user->subject_id = $input['subject'][$key];
            $user->year = $input['year'];
            $user->applicant_id = $input['applicant_id'];
            $user->reg_number = $input['reg_number'];
            $user->exam_type = $input['exam_type'];
            $user->save();
        }

        // $olevelinput = $request->all();
        // $studentolevel = Applicant::where('id', Auth::user()->applicant_id)->first();
        // $studentolevel['is_complete'] = 3;
    ...
    public function create_publicationreg(Request $request){
        $input = $request->all();
        $input['applicant_id'] = Auth::user()->applicant_id;
        $publication = Publication::where('applicant_id', Auth::user()->applicant_id)->first();
        $this->validate($request, [
            'title'=> 'required',
    ...
    public function create_certificatereg(Request $request){
        ...
        $StudentCertificate = StudentCertificate::where('applicant_id', Auth::user()->applicant_id)->first();
            ...
            $studinput['certificate'] = $fileName = time() . '.' . $request->certificate->extension();
            $request->certificate->move(public_path('student_uploaded_document'), $fileName);
    ...
    public function profile_pics(Request $request){
        ...
        if ($student) {
            ...
            $input['profile_picture'] = $fileName = time() . '.' . $request->profile_picture->extension();
            $request->profile_picture->move(public_path('student_uploaded_document'), $fileName);
           $student->update([
